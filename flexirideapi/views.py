...
from flexirideapi.number_plate_detection import NumberPlateDetection
from flexirideapi.cardamage import CarDamageDetection
from flexirideapi.car_model import StanfordCarsNet
import torch
...
class ImageAPIView(APIView):
    ...
    def get(self, request, format= None):
        print(request.data)
        response = Response({"message": "This is FlexirideAPI API"})
        return response
    ...
    def post(self, request, *args, **kwargs):
        ...
        _, data = file
        print(os.getcwd())
        path = "media/{}".format(data[0])
        with open(path, "wb+") as f:
                for chunk in data[0].chunks():
                    f.write(chunk)
        
        detector = NumberPlateDetection()
        outputString= detector.detectNumberPlate(path)


        # model = StanfordCarsNet.load_from_checkpoint("flexiride/flexirideapi/ai_models/epoch=70-step=3621.ckpt", is_finetuned = True)
        model =StanfordCarsNet.load_from_checkpoint('flexirideapi/ai_models/epoch=61-step=6324.ckpt', map_location=torch.device("cpu"), is_finetuned = True)
        ...
        class_probs = softmax(prediction)
        ...
        jsonString = {'number_plate': outputString, "car_model": classes[predicted_class.item()]}

        if predicted_class.item() is not None :
            return Response(jsonString, status=200, headers={'Content-Type': 'application/json'})
        else:
            return Response(json.dumps({"message": "Image Not Found"}), status=200, headers={'Content-Type': 'application/json'})
